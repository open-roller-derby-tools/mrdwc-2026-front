<template>
	...
	>
		...
		>
			<div v-if="member.number && !isStaffLike" class="text-red-light font-bold text-lg">
				#{{ member.number || "—" }}
			</div>
			...
			<!-- BENCH STAFF -->
			<div v-if="isBenchStaff" class="text-lg text-red-light font-semibold uppercase tracking-wide">
				Bench Staff
			</div>
		</div>
	</div>

<template>
	<div class="">
		...
		<div class="bg-white text-blue-text rounded-b-2xl p-6">
			...
			<Venue v-for="(venue, i) in venuesStore.localizedVenues" :key="`venue_${i}`" :venue="venue" />
		</div>
	</div>
</template>
...
const venuesWithImages = computed(() => venuesStore.localizedVenues.filter((venue) => venue.image));
</script>

  ...
  const relevantGames =
    currentViewType.value === "timeGridDay"
      ? allGames.filter((game) => {
          const gameDay = format(
            new TZDate(game.start_time, active_timezone.value),
            "yyyy-MM-dd",
            { in: activeTz },
          );
...
const syncTrackButtonClasses = () => {
  ...
  trackButtonNames.value.forEach((buttonName) => {
    const buttonEl = calendarRoot.querySelector<HTMLElement>(
...
const calendarOptions = computed<CalendarOptions>(() => {
  const viewButtons = smOrSmaller.value
    ? "timeGridDay"
...
onMounted(() => {
  ...
  nextTick(() => {
    currentViewType.value =
      calendarRef.value?.getApi().view.type ?? currentViewType.value;

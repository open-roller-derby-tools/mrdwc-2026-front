<template>
	<div class="flex items-center gap-1 text-sm font-medium" :class="gameTimeOrStateClass">
		...
		<span class="overflow-hidden text-ellipsis whitespace-nowrap first-letter:uppercase">{{
...
const gameTimeOrStateLabel = computed(() => {
	switch (props.game.state) {
		...
		case GameState.PreGame:
			if (!props.showDay && !props.showTime) return t("game_state.scheduled");
			const parts = [];
			if (props.showDay) parts.push(formatDay(props.game.start_time));
			if (props.showTime) parts.push(formatTime(props.game.start_time));
			return parts.join(", ");

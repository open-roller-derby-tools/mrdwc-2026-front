<template>
	<div :id="data.anchor_id" :class="[data.classes, 'relative']">
		...
		<div class="sm:hidden relative z-10 maxed mb-6">
			<div v-for="slug in data.tabs" :key="slug" class="mt-0.5 first:mt-0">
				<button
					:data-slug="slug"
					:class="[
						...
						openSlug === slug ? 'bg-yellow text-blue-text' : 'bg-blue-light text-white',
					]"
...
const toggleTab = async (slug: string) => {
	...
	if (!wasOpen && openSlug.value) {
		...
		const buttonEl = document.querySelector(`button[data-slug="${slug}"]`) as HTMLElement | null;
		if (!buttonEl) {
			return;
		}
		...
		const top = buttonEl.getBoundingClientRect().top + window.scrollY - headerOffset;

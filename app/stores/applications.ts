...
export const useApplicationsStore = defineStore("applications", () => {
	...
	async function fetch() {
		...
		try {
			...
			const { data } = await $fetch<IApplicationsRequestData>(
				buildRESTURL(apiBase, "applications", fields).href
			);
	...
	const localizedApplications = computed((): ILocalizedApplication[] => {
		...
		return applications.value.reduce<ILocalizedApplication[]>((result, application) => {
	...
	const getApplicationsWithStatus = computed(() => {
		return (statuses: ("upcoming" | "open" | "closed")[]): ILocalizedApplication[] => {

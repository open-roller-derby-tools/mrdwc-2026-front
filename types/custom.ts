...
export interface ILocalizedMenu {
	...
	classes: string;
	display_name: string;
	items: (ILocalizedPageMenuItem | ILocalizedCustomLinkMenuItem | ILocalizedMenuMenuItem)[] | null;
}
...
export interface IBlockWrapper {
	sort: number;
	collection: string;
	item: IBlockRichText | IBlockCustom | IBlockTwoColumns | IBlockTabs | IBlockTabsSlot;
}
...
export interface IOfficialsCrew {
	...
	secret_name: string;
	color: string;
	members_so: IOfficialId[];
	members_nso: IOfficialId[];
}

export interface ILocalizedOfficialsCrew {
	id: number;
	name: string;
	secret_name: string;
	color: string;

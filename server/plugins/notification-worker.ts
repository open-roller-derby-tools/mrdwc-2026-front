...
export default defineNitroPlugin(() => {
	...
	const worker = new Worker(
		...
		async (job) => {
			...
			// Get notification
			const notifRes = await db.query(`SELECT * FROM notifications WHERE id = $1`, [
				notification_id,
			]);
			...
			const usersRes = await db.query(`SELECT user_id FROM subscriptions WHERE channel_id = $1`, [
				notif.channel_id,
			]);
			...
			for (const u of usersRes.rows) {
				const subsRes = await db.query(`SELECT * FROM push_subscriptions WHERE user_id = $1`, [
					u.user_id,
				]);
			...
			if (noUserSubscription) {
				await job.log(
					`[BullMQ] No subscriptions at all for notification ${notification_id}, skipping`
				);
				return;
			}
			...
			if (globalSuccess) {
				await db.query(`UPDATE notifications SET sent_at = NOW() WHERE id = $1`, [notification_id]);
			} else {
